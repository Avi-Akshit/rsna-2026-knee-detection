...
class RSNARadiologyDataset(Dataset):
    ...
    def _read_dicom_slice(self, path: Path) -> np.ndarray:
        try:
            ...
            return np.zeros(cfg.image_size, dtype=np.uint8)

    def _get_volume_stack(self, series_path: Path) -> np.ndarray:
        if not series_path.exists():
            return np.zeros((*cfg.image_size, cfg.in_channels), dtype=np.uint8)
        ...
        indices = np.linspace(0, total_slices - 1, cfg.in_channels, dtype=int)
        stacked_slices = []
